...
def captureFrames():
    global video_frame, thread_lock, detect, detection
    ...
    while detect and video_capture.isOpened():
...
def runMain():
    ...
    # MyStatics.loop = loop
    ops = CharOperation()
    detection = Detection(ops.getQuad())
    hand = CharHandler(ops,startCapturing,stopCapturing)
...
if __name__ == '__main__': 
    ...
    threading.Thread(target=lambda:runMain()).start()
    # start the Flask Web Application
    # While it can be run on any feasible IP, IP = 0.0.0.0 renders the web app on
    # the host machine's localhost and is discoverable by other machines on the same network 
    app.run("0.0.0.0", port="8008")
